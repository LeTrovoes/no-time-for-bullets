...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            Vector3 newRotation = Quaternion.Lerp(transform.rotation, rollRotation, m_TurnSpeed*m_CurrentTurnAmount*deltaTime).eulerAngles;

            // OUR CODE
            var controlY = Input.GetAxis("Mouse Y");
            m_TiltAngle -= controlY*2;
            m_TiltAngle = Mathf.Clamp(m_TiltAngle, -10, 20);

            // ROTATE CAMERA
            // set height to 1.8
            transform.rotation = Quaternion.Euler(newRotation.x, newRotation.y, newRotation.z);
            mainCamera.rotation = Quaternion.Euler(m_TiltAngle, mainCamera.rotation.eulerAngles.y, mainCamera.rotation.eulerAngles.z);

            // ROTATE CAMERA AROUND
            // set height 1.6
            // transform.rotation = Quaternion.Euler(m_TiltAngle, newRotation.y, newRotation.z);
        }
    }
}
